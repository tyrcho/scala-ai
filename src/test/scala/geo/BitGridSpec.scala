...
class BitGridSpec extends FlatSpec with Matchers {
  ...
  "a BitGrid" should "initially be empty" in {
    val grid = GridData(3)
    Masks.isComplete(grid, 3) shouldBe false
    grid.empty shouldBe true
  }

  it should "detect a full row" in {
    val grid = GridData(3) + (0, 0) + (0, 1) + (0, 2)
    Masks.isComplete(grid, 3) shouldBe true
  }

  it should "detect a full row with extra" in {
    val bg = GridData(3) + (0, 0) + (0, 1) + (0, 2) + (1, 0)
    bg.empty shouldBe false
    Masks.isComplete(bg, 3) shouldBe true
  }

  it should "detect a diag1" in {
    val bg = GridData(3) + (0, 0) + (1, 1) + (2, 2)
    bg.empty shouldBe false
    Masks.isComplete(bg, 3) shouldBe true
  }

  it should "detect a diag2" in {
    val bg = GridData(3) + (0, 2) + (1, 1) + (2, 0)
    bg.empty shouldBe false
    Masks.isComplete(bg, 3) shouldBe true
  }

  it should "detect incomplete" in {
    val bg = GridData(3) + (0, 1) + (1, 1) + (2, 0)
    bg.empty shouldBe false
    Masks.isComplete(bg, 3) shouldBe false
  }

  for (c <- 0 to 2) {
    it should s"detect a full col in pos $c" in {
      val empty = GridData(3)
  ...
  it should "detect a full col in a large grid" in {
    val empty = GridData(19)
    val bg = empty.addCol(11)
    bg.empty shouldBe false
    Masks.isComplete(bg, 19) shouldBe true
  }
  ...
  it should "detect a full row - 1 in a large grid" in {
    ...
    bg.empty shouldBe false
    Masks.isComplete(bg, 19) shouldBe false
  }
  ...
  it should "detect a partial row " in {
    ...
    bg.empty shouldBe false
    Masks.isComplete(bg, 5) shouldBe true
  }
  ...
  it should "detect a partial col" in {
    ...
    bg.empty shouldBe false
    Masks.isComplete(bg, 6) shouldBe true
  }
