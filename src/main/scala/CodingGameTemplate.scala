...
object CodinGameTemplate {
  ...
  implicit class VectorOps[T](vector: Vector[T]) {
    def updatef(index: Int, f: T => T): Vector[T] =
      vector.updated(index, f(vector(index)))
  }

  implicit class VectorOps2[T](vector: Vector[Vector[T]]) {
    def updatef2(i: Int, j: Int, f: T => T): Vector[Vector[T]] =
